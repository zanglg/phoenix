...
| RUN-INIT-001 | Execute the separately linked native init ELF | Runtime Pending | `cargo xtask test-init` selects the exact inspected RX image from the embedded initramfs, loads it through the DTB allocator and dynamic tables, enters at `0x00400000`, validates its stack, and reaches `PHOENIX_INIT_OK` through kernel `exit(42)` |
