...
| RUN-ASID-001 | Publish a nonzero process ASID | Runtime Pending | GDB observes ASID 1 in TTBR0 for both EL0 variants, address translation survives the switch, and ASID zero is not used for process mappings |
| RUN-KSTACK-001 | Final boot-stack guard translation fault | Runtime Pending | After final TTBR1 publication, GDB confirms the guard has no valid descriptor and a controlled access reports the expected EL1 data abort without corrupting the live stack |
...
| RUN-INIT-001 | Execute the separately linked native init ELF | Runtime Pending | `cargo xtask test-init` observes ordered final-TTBR1 enter/success and EL0-enter markers, selects the exact inspected RX image, publishes TTBR0 with ASID 1, validates its stack, emits its greeting and exact `etc/motd` bytes through checked I/O, then transitions Running to Exited(42) and reaches `PHOENIX_INIT_OK` |
| RUN-EL0-001 | Enter and return from the first EL0 program | Runtime Pending | `cargo xtask test-el0` reaches `PHOENIX_EL0_OK` after an ASID-1 TTBR0 switch, EL0 `eret`, returning unknown syscall, and `exit(42)` |
